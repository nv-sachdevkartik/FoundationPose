...
class TensorRTInfer:
    ...
    def infer_batch(self, image1_batch: np.ndarray, image2_batch: np.ndarray) -> np.ndarray:
        """
        Run inference on batches of two input images.

        Args:
            image1_batch (np.ndarray): First batch of images of shape (batch_size, 3, 160, 160)
            image2_batch (np.ndarray): Second batch of images of shape (batch_size, 3, 160, 160)

        Returns:
            np.ndarray: Model output
        """
        return self.infer(image1_batch, image2_batch)
...
if __name__ == "__main__":
    engine_paths = [
        "/home/ksachdev/repos/FoundationPose/weights/2023-10-28-18-33-37/model_best_dynamic.plan",
        "/home/ksachdev/repos/FoundationPose/weights/2024-01-11-20-02-45/model_best_dynamic.plan"
    ]
